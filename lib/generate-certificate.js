...
function generateCertificate(options) {
    ...
    return function (recipient) {
        return new Promise(function (resolve, reject) {
            ...
            if (options.font) {
                img = img.font(options.font.face, options.font.size).stroke(options.font.color).fill(options.font.color);
            }

            if (Array.isArray(options.mergeVars)) {
                options.mergeVars.forEach(function (v) {
                    if (recipient[v.key]) {
                        img.region(v.box.width, v.box.height, v.box.left, v.box.top).drawText(0, 0, recipient[v.key], options.font.gravity);
                    }
                });
            }
